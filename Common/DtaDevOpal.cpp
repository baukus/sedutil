...
{
	LOG(D1) << "Entering DtaDevOpal::setPassword userid " << userid ;
	uint8_t lastRC;
	...
	uint8_t hasholdpwd, uint8_t hashnewpwd)
{
	...
	char *opwd = oldpassword, *npwd = newpassword;
	...
	if ((oldpassword == NULL) || (*oldpassword == '\0') ||
		...
		defaultPassword = response.getString(4);
		if ((oldpassword == NULL) || (*oldpassword == '\0')) {
			opwd = (char *)defaultPassword.c_str();
			hasholdpwd = 0;
		}

		if ((newpassword == NULL) || (*newpassword == '\0')) {
			npwd = (char *)defaultPassword.c_str();
			hashnewpwd = 0;
		}
	}
	...
	hash.clear();
	if (hashnewpwd) {
		DtaHashPwd(hash, npwd, this);
	}
	else {
		hash.push_back(0xd0);
		hash.push_back((uint8_t)strnlen(npwd, 255));
		for (uint16_t i = 0; i < strnlen(npwd, 255); i++) {
			hash.push_back(npwd[i]);
		}
	}
